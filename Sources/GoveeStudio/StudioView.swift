...
struct StudioView: View {
    ...
    var body: some View {
        ...
        .alert("Govee Studio",isPresented:Binding(get:{model.message != nil},set:{if !$0 { model.message=nil }})) {
            if model.message?.contains("Bildschirmaufnahme") == true {
                Button("Einstellungen öffnen") { model.openScreenPrivacy() }
            }
    ...
    private var header: some View {
        HStack(spacing:14) {
            ...
            VStack(alignment:.leading,spacing:3) {
                Text("Govee Studio").font(.system(size:16,weight:.semibold))
                Text("DESKTOP / LIGHT LAB").font(.system(size:8,weight:.medium,design:.monospaced)).tracking(1.7).foregroundStyle(Palette.muted)
            }
            HStack(spacing:4) {
                ForEach(ExperienceMode.allCases,id:\.self) { mode in
                    Button { model.activateMode(mode) } label: { Label(mode.rawValue,systemImage:mode.icon) }
                        .buttonStyle(StudioButtonStyle(prominent:!model.setupTab && (model.requestedMode ?? model.show.mode) == mode))
                }
                Button { model.setupTab=true } label: { Label("Setup",systemImage:"slider.horizontal.3") }
            ...
            HStack(spacing:7) {
                ...
                Text(statusText).font(.system(size:11,weight:.medium,design:.monospaced)).foregroundStyle(model.runState == .syncing ? Palette.mint:Palette.muted)
            }.padding(.horizontal,12).padding(.vertical,8).background(Palette.raised,in:Capsule())
            if model.busy {
                Button { Task { await model.stop() } } label: {
                    Label(model.runState == .starting ? "Abbrechen":"Stoppen",systemImage:"stop.fill")
                ...
                Text("Effekt auswählen").font(.system(size:11)).foregroundStyle(Palette.muted)
            } else {
                Button { model.begin(output:false) } label: { Label("Vorschau",systemImage:"eye") }
                    .buttonStyle(StudioButtonStyle())
                Button { model.begin(output:true) } label: { Label(model.show.mode == .ambience ? "Sync starten":"Licht starten",systemImage:"play.fill") }
    ...
    private var footer: some View {
        HStack(spacing:8) {
            ...
            Text(model.activeCloud == nil ? "Lokal auf deinem Mac":"Govee-Geräteeffekt").font(.system(size:10,weight:.medium))
            Text("·").foregroundStyle(Palette.muted)
            Text(model.notice).font(.system(size:10)).foregroundStyle(Palette.muted).lineLimit(1)
            Spacer()
            Text("v0.8.1 · Created by Naiklas").font(.system(size:9,design:.monospaced)).foregroundStyle(Palette.muted)
    ...
    var body: some View {
        VStack(alignment:.leading,spacing:0) {
            HStack {
                ...
                Spacer()
                Text("\(model.onlineCount)/\(model.devices.count)").font(.system(size:10,design:.monospaced)).foregroundStyle(Palette.mint)
                ...
                VStack(spacing:9) {
                    ...
                    if model.devices.isEmpty {
                        VStack(spacing:14) {
                            ...
                            Text(model.scanning ? "Suche deine Leuchten …":"Noch keine Leuchten gefunden").font(.system(size:12,weight:.medium))
                            Text("LAN-Steuerung in Govee Home aktivieren. Mac und Leuchten brauchen dasselbe Netzwerk.").font(.system(size:11)).foregroundStyle(Palette.muted).multilineTextAlignment(.center)
                        }.padding(.vertical,28).padding(.horizontal,10)
                    }
                }.padding(.horizontal,12)
            }
            Button { Task { await model.scan() } } label: {
                HStack { Image(systemName:model.scanning ? "antenna.radiowaves.left.and.right":"arrow.clockwise"); Text(model.scanning ? "Suche läuft …":"Geräte suchen"); Spacer() }
            ...
            VStack(alignment:.leading,spacing:13) {
                ...
                HStack(spacing:8) {
                    TextField("Profilname",text:$model.profileName).textFieldStyle(.plain).font(.system(size:12)).padding(9).background(Palette.raised,in:RoundedRectangle(cornerRadius:7))
                    Button { model.saveProfile() } label: { Image(systemName:"square.and.arrow.down").font(.system(size:14)) }.buttonStyle(.plain).help("Aktuelles Setup als Profil speichern")
                }
                Menu {
                    if model.profiles.isEmpty { Text("Noch keine gespeicherten Profile") }
                    ForEach(model.profiles) { profile in Button(profile.name) { model.loadProfile(profile) } }
                } label: { HStack { Image(systemName:"square.stack"); Text("Profil laden"); Spacer(); Image(systemName:"chevron.down") }.font(.system(size:11)).foregroundStyle(Palette.muted) }
                .menuStyle(.borderlessButton).disabled(model.busy)
                Text("Änderungen werden automatisch gesichert.").font(.system(size:10)).foregroundStyle(Palette.muted).fixedSize(horizontal:false,vertical:true)
            }.padding(20)
            ...
            VStack(alignment:.leading,spacing:13) {
                HStack(spacing:10) {
                    ...
                    VStack(alignment:.leading,spacing:4) {
                        ...
                        Text(device.sku).font(.system(size:10,design:.monospaced)).foregroundStyle(Palette.muted)
                    }
                    Spacer(minLength:0)
                    Circle().fill(model.isOnline(device) ? Palette.mint:Palette.muted.opacity(0.4)).frame(width:5,height:5)
                }
                HStack {
                    Text(device.enabled ? "\(device.mode == .whole ? 1:device.zones.count) Zonen":"Pausiert").foregroundStyle(device.enabled ? tint:Palette.muted)
                    Spacer()
                    Text(device.ip).foregroundStyle(Palette.muted)
