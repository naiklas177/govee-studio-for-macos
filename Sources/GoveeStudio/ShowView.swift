...
struct ShowView: View {
        ...
        HStack(alignment:.top,spacing:0) {
                ...
                VStack(alignment:.leading,spacing:22) {
                    HStack(alignment:.top) {
                        VStack(alignment:.leading,spacing:9) {
                            ...
                            Text(verbatim:tr(model.show.mode == .ambience ? "Dein Bildschirm.\nEin ganzer Raum." : (model.show.mode == .music ? "Dein Sound.\nDein Licht.":(model.browseCloud ? "Originale.\nNeue Eskalation.":model.show.scene.rawValue + ".\nLicht mit Charakter."))))
                                .font(.system(size:36,weight:.semibold)).tracking(-1.4)
                            Text(verbatim:tr(model.show.mode == .ambience ? "Farben direkt vom Desktop. Dein Mapping wartet im Setup." : (model.show.mode == .music ? "Reaktion und Farben direkt auf deine Musik abstimmen.":(model.browseCloud ? "Die Govee-Bibliothek für deine Geräte.":model.show.scene.subtitle))))
                                .font(.system(size:12)).foregroundStyle(Palette.muted)
                        }
                        Spacer()
                        Text(verbatim:tr(model.show.mode == .music ? "AUDIO → LICHT" : "LOKALE LICHTSZENEN"))
                    ...
                    if let plan=model.activeRoom {
                        ...
                        VStack(alignment:.leading,spacing:10) {
                            Label(tr("Govee-Effekt · Befehl bestätigt"),systemImage:"cloud.fill").foregroundStyle(Palette.mint)
                            Text(scene.displayName).font(.system(size:22,weight:.semibold))
                            Text(verbatim:tr("Auf " + (model.devices.first(where:{$0.id == scene.device})?.name ?? scene.sku) + " · Die Animation wird vom Gerät dargestellt. Keine Live-Vorschau verfügbar.")).font(.system(size:11)).foregroundStyle(Palette.muted)
                    ...
                    if model.show.mode == .music {
                        Picker(tr("Musikquelle"),selection:$model.nativeMusic) {
                            Text(verbatim:tr("Mac · Systemaudio")).tag(false)
                            Text(verbatim:tr("Govee · Geräte-Musik")).tag(true)
                        }.pickerStyle(.segmented)
                        if model.nativeMusic { NativeMusicView(library:model.cloud) }
                        else { MusicDeck(live:live) }
                    } else if model.show.mode == .scenes {
                        Picker(tr("Szenenquelle"),selection:$model.browseCloud) {
                            Text(verbatim:tr("Studio · Lokal")).tag(false)
                            Text(verbatim:tr("Govee · Original & DIY")).tag(true)
                        }.pickerStyle(.segmented)
                        if model.browseCloud { CloudLibraryView(library:model.cloud) } else {
                        HStack { Eyebrow(text:"Wähle deinen Zustand"); Spacer(); Text(verbatim:tr("LIVE WECHSELBAR")).font(.system(size:8,design:.monospaced)).foregroundStyle(Palette.muted) }
                        ...
                        HStack(spacing:18) {
                            ...
                            VStack(alignment:.leading,spacing:5) {
                                Text(verbatim:tr("Dein Mapping bleibt dein Mapping.")).font(.system(size:15,weight:.medium))
                                Text(verbatim:tr("Zonen verschieben, Geräte zuordnen und kalibrieren: alles im Setup-Tab.")).font(.system(size:11)).foregroundStyle(Palette.muted)
                            }
                            Spacer()
                            Button(tr("Setup öffnen")) { model.setupTab=true }.buttonStyle(StudioButtonStyle())
            ...
            if model.show.mode == .music && model.nativeMusic {
                ...
                VStack(alignment:.leading,spacing:16) {
                    ...
                    Text(verbatim:tr("Der Effekt übernimmt.")).font(.system(size:21,weight:.medium))
                    Text(verbatim:tr("Original- und DIY-Effekte besitzen ihre eigenen Farben und Abläufe. Unsere Regler für Musik und lokale Szenen wirken hier nicht.")).font(.system(size:12)).foregroundStyle(Palette.muted)
                    Text(verbatim:tr("Pro Leuchte übernimmt genau eine Quelle. Im Raum-Mix können Govee-Gerätemusik und Ambilight auf verschiedenen Leuchten gleichzeitig laufen. Änderungen in der Zuordnung werden beim Start übernommen.")).font(.system(size:11)).foregroundStyle(Palette.muted)
                    Spacer()
    ...
    private func sceneCard(_ scene: LightScene) -> some View {
        ...
        return Button { model.chooseScene(scene) } label: {
            VStack(alignment:.leading,spacing:10) {
                ...
                HStack { Text(verbatim:tr(scene.rawValue)).font(.system(size:13,weight:.semibold)); Spacer(); if selected { Image(systemName:"checkmark.circle.fill").foregroundStyle(Palette.mint) } }
                Text(verbatim:tr(scene.subtitle)).font(.system(size:10)).foregroundStyle(Palette.muted).lineLimit(2).frame(height:28,alignment:.topLeading)
        ...
        ScrollView {
            VStack(alignment:.leading,spacing:22) {
                ...
                VStack(alignment:.leading,spacing:8) {
                    Text(verbatim:tr(model.show.mode == .music ? "Der Bass darf rein." : "Du hast den Regler.")).font(.system(size:21,weight:.medium)).tracking(-0.5)
                    Text(verbatim:tr(model.show.mode == .music ? "Mac-Systemaudio · kein Mikrofon. Bass, Mitten und Höhen treiben unterschiedliche Kanäle." : "Lokale Ausgabe auf deinem gesamten aktivierten Setup."))
                ...
                if model.show.mode != .ambience {
                    ThinDivider()
                    if model.show.mode == .music {
                        Picker(tr("Hintergrund"),selection:Binding(get:{model.show.musicBackground.enabled},set:{model.show.musicBackground.enabled=$0;model.showChanged()})) {
                            Text(verbatim:tr("Farbe")).tag(false)
                            Text(verbatim:tr("Ambilight")).tag(true)
                        }.pickerStyle(.segmented).disabled(model.busy)
                        if model.busy { Text(verbatim:tr("Zum Wechsel der Hintergrundquelle kurz stoppen.")).font(.system(size:9)).foregroundStyle(Palette.muted) }
                    }
                    if model.show.mode == .music && model.show.musicBackground.enabled {
                        ...
                        Toggle(tr("Filmbalken ausblenden"),isOn:Binding(get:{model.show.musicBackground.cropBars},set:{model.show.musicBackground.cropBars=$0;model.showChanged()})).font(.system(size:10))
                        Text(verbatim:tr("Bildschirm und Zonen aus Setup. Musik und Hintergrund werden separat geglättet.")).font(.system(size:10)).foregroundStyle(Palette.muted)
                    } else {
                    ...
                    if (model.show.mode == .scenes && (model.show.scene == .meteor || model.show.scene == .implosion)) || (model.show.mode == .music && (model.show.musicPattern == .chase || model.show.musicPattern == .wave)) {
                        ...
                        Toggle(tr("Effektrichtung umkehren"),isOn:Binding(get:{model.show.reverseMotion},set:{model.show.reverseMotion=$0;model.showChanged()})).font(.system(size:10))
                        Text(verbatim:tr("Bewegung folgt der Kanalfolge pro Gerät.")).font(.system(size:10)).foregroundStyle(Palette.muted)
                    }
                }
                if model.show.mode == .music {
                    ThinDivider()
                    ...
                    Text(verbatim:tr(model.show.musicBackground.enabled ? "Bei Stille läuft Ambilight weiter. Musik liegt darüber; Master und Gerätehelligkeit begrenzen die Gesamtausgabe.":"Bei Stille bleibt nur dein Grundlicht. Die Vorschau hört ebenfalls Systemaudio, sendet aber nichts an die Leuchten.")).font(.system(size:10)).foregroundStyle(Palette.muted)
                }
                ThinDivider()
                HStack { Eyebrow(text:"Verbunden"); Spacer(); Text(verbatim:tr("\(model.onlineCount)/\(model.devices.count)")).foregroundStyle(Palette.mint).monospacedDigit() }
                ForEach(model.devices) { device in
                    HStack { Circle().fill(model.isOnline(device) ? Palette.mint:Palette.muted).frame(width:5,height:5); Text(device.name).font(.system(size:11)); Spacer(); Text(verbatim:tr(device.enabled ? "AN":"AUS")).font(.system(size:9,design:.monospaced)).foregroundStyle(Palette.muted) }
                }
                Text(verbatim:tr("Stoppen setzt die gespeicherten Grundwerte deiner Leuchten zurück.")).font(.system(size:10)).foregroundStyle(Palette.muted)
            }.padding(22)
    ...
    private func slider(_ title: String,value: Binding<Double>,range: ClosedRange<Double>,display: String) -> some View {
        VStack(spacing:9) {
            HStack { Text(verbatim:tr(title)).font(.system(size:11,weight:.medium)); Spacer(); Text(verbatim:tr(display)).font(.system(size:10,design:.monospaced)).foregroundStyle(Palette.mint) }
...
struct MusicDeck: View {
    ...
    var body: some View {
        VStack(alignment:.leading,spacing:18) {
            ...
            Text(verbatim:tr("Mac-Reaktionen laufen über Panel- und Strip-Kanäle. Einzelne Hexagon-Teilflächen sind hier noch nicht frei adressierbar.")).font(.system(size:10)).foregroundStyle(Palette.muted)
            HStack(alignment:.top,spacing:24) {
                        ...
                        Button { model.show.musicPattern=pattern;model.showChanged(); if model.activeCloud != nil { model.activateMode(.music) } } label: {
                            HStack(spacing:12) {
                                ...
                                VStack(alignment:.leading,spacing:4) {
                                    Text(verbatim:tr(pattern.rawValue)).font(.system(size:12,weight:.semibold))
                                    Text(verbatim:tr(pattern.detail)).font(.system(size:10)).foregroundStyle(Palette.muted).fixedSize(horizontal:false,vertical:true)
                                }
                                Spacer()
                ...
                VStack(alignment:.leading,spacing:20) {
                    ...
                    Picker(tr("Farbverhalten"),selection:Binding(get:{model.show.musicColorFlow},set:{model.show.musicColorFlow=$0;model.showChanged()})) {
                        ForEach(MusicColorFlow.allCases,id:\.self) { flow in Text(verbatim:tr(flow.rawValue)).tag(flow) }
                    }.labelsHidden().accessibilityLabel(tr("Farbverhalten"))
                    if model.show.musicSaturation < 0.5 {
                        Button(tr("Satte Farben aktivieren")) { model.show.musicSaturation=1.1;model.showChanged() }.buttonStyle(StudioButtonStyle())
                        Text(verbatim:tr("Aktuell \(Int(model.show.musicSaturation*100)) % Sättigung – Farben sind fast weiß.")).font(.system(size:9)).foregroundStyle(Palette.muted)
                    }
                    ForEach(MusicColors.allCases,id:\.self) { colors in
                        Button { model.show.musicColors=colors;model.showChanged() } label: {
                            HStack(spacing:6) {
                                ...
                                Text(verbatim:tr(colors.rawValue)).font(.system(size:11)).padding(.leading,4)
                                Spacer()
...
struct AudioMeter: View {
    ...
    private func bar(_ title: String,level: Double,color: Color) -> some View {
        VStack(spacing:7) {
            ...
            Text(verbatim:tr(title)).font(.system(size:8,design:.monospaced)).foregroundStyle(Palette.muted)
            Text(verbatim:tr(level > 0.00001 ? String(format:"%.0f dB",20*log10(level)):"−∞ dB")).font(.system(size:8,design:.monospaced)).foregroundStyle(Palette.muted)
        }
    }
}
        ...
        TimelineView(.animation(minimumInterval:0.05,paused:reduceMotion || model.running)) { context in
            VStack(alignment:.leading,spacing:14) {
                HStack {
                    ...
                    Spacer()
                    Text(verbatim:tr(model.running ? String(format:"%.1f FPS",live.fps):"READY")).font(.system(size:9,design:.monospaced)).foregroundStyle(Palette.mint)
                }
                Canvas { canvas,size in
                    ...
                    if devices.isEmpty { canvas.draw(Text(verbatim:tr("Dein Licht wartet im Setup.")).foregroundColor(Palette.muted),at:CGPoint(x:size.width/2,y:size.height/2)) }
                }
            }.padding(20)
    ...
    var body: some View {
        if let device=model.selected,device.isHex {
            VStack(alignment:.leading,spacing:12) {
                ...
                Text(verbatim:tr("Flächen & Linien erkunden")).font(.system(size:14,weight:.medium))
                Text(verbatim:tr("Experimentelle Kanalzahl für Szenen und Musik. Die Zuordnung zu Teilflächen ist noch nicht bestätigt; mehr Kanäle bedeuten nicht automatisch mehr adressierbare LEDs."))
                ...
                Stepper(value:Binding(get:{model.show.count(for:device)},set:{model.show.channelCounts[device.id]=$0;model.probeChannel=min(model.probeChannel,$0-1);model.showChanged()}),in:1...84) {
                    Text(verbatim:tr("\(model.show.count(for:device)) Show-Kanäle")).font(.system(size:11))
                }.disabled(model.busy)
                Stepper(value:$model.probeChannel,in:0...max(0,model.show.count(for:device)-1)) { Text(verbatim:tr("Testkanal \(model.probeChannel+1)")).font(.system(size:11)) }.disabled(model.busy)
                Button(tr("Kanal 1,5 Sekunden markieren")) { model.beginChannelProbe() }.buttonStyle(StudioButtonStyle()).disabled(model.busy)
                Text(verbatim:tr("Türkis auf dunklem Grund, danach Wiederherstellung. Ambilight-Zonen bleiben unverändert.")).font(.system(size:9)).foregroundStyle(Palette.muted)
                Button(tr("Show-Kanalzahl zurücksetzen")) { model.show.channelCounts.removeValue(forKey:device.id);model.probeChannel=0;model.showChanged() }.buttonStyle(.plain).font(.system(size:10)).foregroundStyle(Palette.mint).disabled(model.busy)
            }
        }
    }
}
