...
struct InspectorView: View {
            ...
            VStack(alignment:.leading,spacing:20) {
                ...
                if model.recoveryAvailable {
                    VStack(alignment:.leading,spacing:10) {
                        Text(verbatim:tr("Vorherige Sitzung")).font(.system(size:12,weight:.semibold))
                        Text(verbatim:tr("Gesicherte Lichtwerte sind noch vorhanden.")).font(.system(size:10)).foregroundStyle(Palette.muted)
                        Button(tr("Lichtwerte wiederherstellen")) { Task { await model.recoverPrevious() } }
    ...
    private func deviceControls(_ device: DeviceConfig) -> some View {
        VStack(alignment:.leading,spacing:15) {
            TextField(tr("Gerätename"),text:Binding(get:{model.selected?.name ?? ""},set:{value in model.editSelected{$0.name=value}}))
                .font(.system(size:18,weight:.medium)).textFieldStyle(.plain)
            HStack {
                Text(verbatim:tr("Im Sync verwenden")).font(.system(size:11))
                Spacer()
                Toggle(tr("Im Sync verwenden"),isOn:Binding(get:{model.selected?.enabled ?? false},set:{value in model.editSelected{$0.enabled=value}}))
            ...
            HStack {
                Text(verbatim:tr("Segmente")).font(.system(size:11))
                Spacer()
                Stepper(value:Binding(get:{model.selected?.zones.count ?? 1},set:{model.setCount($0)}),in:1...84) {
                    Text(verbatim:tr("\(device.zones.count)")).font(.system(size:12,design:.monospaced)).frame(minWidth:22)
                }.fixedSize().disabled(model.busy || device.mode == .whole)
            }
            if !device.calibrated {
                Text(verbatim:tr("Startwert · Anzahl und Reihenfolge am Gerät prüfen."))
                    .font(.system(size:10)).foregroundStyle(Color.orange.opacity(0.85)).fixedSize(horizontal:false,vertical:true)
            }
            Toggle(tr("Reihenfolge umkehren"),isOn:Binding(get:{model.selected?.reverse ?? false},set:{value in model.editSelected{$0.reverse=value}}))
                .font(.system(size:11)).toggleStyle(.checkbox)
            DisclosureGroup(isExpanded:$advanced) {
                VStack(alignment:.leading,spacing:12) {
                    Picker(tr("Ausgabe"),selection:Binding(get:{model.selected?.mode ?? .whole},set:{value in model.editSelected{$0.mode=value}})) {
                        ForEach(OutputMode.allCases,id:\.self) { Text(verbatim:tr($0.rawValue)).tag($0) }
                    }.font(.system(size:10)).disabled(model.busy)
                    if device.mode == .dreamview {
                        Picker(tr("Protokoll"),selection:Binding(get:{model.selected?.header ?? .automatic},set:{value in model.editSelected{$0.header=value}})) {
                            ForEach(StreamHeader.allCases,id:\.self) { Text(verbatim:tr($0.label)).tag($0) }
                        }.font(.system(size:10)).disabled(model.busy)
                        Toggle(tr("Auf Gerät strecken"),isOn:Binding(get:{model.selected?.stretch ?? false},set:{value in model.editSelected{$0.stretch=value}}))
                            .font(.system(size:10)).toggleStyle(.checkbox).disabled(model.busy)
                    }
                    Toggle(tr("Zuordnung geprüft"),isOn:Binding(get:{model.selected?.calibrated ?? false},set:{value in model.editSelected{$0.calibrated=value}}))
                        .font(.system(size:10)).toggleStyle(.checkbox)
                    Text(verbatim:tr("DreamView nutzt ein nicht öffentlich dokumentiertes Protokoll. Einzelfarbe ist der einfachere LAN-Modus."))
                        .font(.system(size:10)).foregroundStyle(Palette.muted).fixedSize(horizontal:false,vertical:true)
                }.padding(.top,10)
            } label: { Text(verbatim:tr("Geräteoptionen")).font(.system(size:11)).foregroundStyle(Palette.muted) }
        }
    }
    private func zoneControls(_ device: DeviceConfig) -> some View {
        VStack(alignment:.leading,spacing:14) {
            ...
            Button { model.beginIdentify() } label: {
                HStack { Image(systemName:"lightbulb.max"); Text(verbatim:tr(model.testActive ? "Test läuft …":"Segment kurz testen")); Spacer() }
            }.buttonStyle(StudioButtonStyle()).disabled(model.busy)
            Text(verbatim:tr("Leuchtet kurz türkis. Danach werden die vorherigen Grundwerte zurückgesendet."))
    ...
    private func coordinate(_ title: String,keyPath: WritableKeyPath<SampleZone,Double>) -> some View {
        HStack(spacing:4) {
            Text(verbatim:tr(title)).font(.system(size:9)).foregroundStyle(Palette.muted)
            Spacer(minLength:0)
            TextField(tr(title),value:Binding<Double>(get:{
            ...
            Text(verbatim:tr("%")).font(.system(size:9)).foregroundStyle(Palette.muted)
    ...
    private var globalControls: some View {
        VStack(alignment:.leading,spacing:15) {
            ...
            HStack {
                Text(verbatim:tr("Farbupdates")).font(.system(size:11))
                Spacer()
                Picker(tr("Farbupdates"),selection:Binding(get:{model.settings.fps},set:{model.settings.fps=$0;model.changed()})) {
                    ForEach([10,15,20,25,30,40],id:\.self) { Text(verbatim:tr("\($0) fps")).tag($0) }
                }.labelsHidden().frame(width:88).disabled(model.busy)
            }
            Toggle(tr("Schwarze Filmbalken erkennen"),isOn:Binding(get:{model.settings.cropBars},set:{model.settings.cropBars=$0;model.changed()}))
                .font(.system(size:10)).toggleStyle(.checkbox)
            Text(verbatim:tr("Beim Stoppen: Grundfarbe, Helligkeit und Ein/Aus zurücksetzen. Govee-Szenen können nicht rekonstruiert werden."))
                .font(.system(size:9)).foregroundStyle(Palette.muted).fixedSize(horizontal:false,vertical:true)
        }
    }
    ...
    private func slider(_ title: String,value: Binding<Double>,range: ClosedRange<Double>,display: String) -> some View {
        VStack(spacing:6) {
            HStack { Text(verbatim:tr(title)).font(.system(size:11)); Spacer(); Text(verbatim:tr(display)).font(.system(size:10,design:.monospaced)).foregroundStyle(Palette.muted) }
