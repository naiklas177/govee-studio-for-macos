...
struct RoomControlsView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment:.leading,spacing:20) {
                ...
                Text(verbatim:tr("Dein Raum. Dein Pegel.")).font(.system(size:21,weight:.medium))
                ...
                Text(verbatim:tr("Lichtintensität der Govee-Musikgeräte. Kein zusätzliches Grundlicht in Musikpausen.")).font(.system(size:10)).foregroundStyle(Palette.muted)
                    ...
                    ForEach(RoomColorScheme.allCases,id:\.self) { scheme in
                        Button { library.room.colorScheme=scheme } label: {
                            VStack(alignment:.leading,spacing:8) {
                                ...
                                Text(verbatim:tr(scheme.rawValue)).font(.system(size:10,weight:.medium))
                        ...
                        }.buttonStyle(.plain).accessibilityLabel(tr("Raum-Farbschema \(scheme.rawValue)"))
                    }
                }
                Text(verbatim:tr(library.room.colorScheme == .automatic ? "Govee bestimmt die Farben seiner internen Animation.":"Eine feste Farbe je Musikgerät, als abgestimmtes Schema im Raum verteilt. Die Animation bleibt beim Gerät.")).font(.system(size:10)).foregroundStyle(Palette.muted)
                ...
                if usesScreen {
                    ...
                    Text(verbatim:tr("Nur für die als Ambilight zugeordneten Leuchten.")).font(.system(size:10)).foregroundStyle(Palette.muted)
                }
                ThinDivider()
                Button(tr(model.roomApplying ? "Wird übernommen …":"Regler übernehmen")) { model.applyRoomControls() }.buttonStyle(StudioButtonStyle(prominent:true)).disabled(model.activeRoom == nil || model.roomApplying || model.switchingMode)
                Text(verbatim:tr("Während der Ausgabe ohne kompletten Neustart. Neue Gerätezuordnung: Raum-Musik starten.")).font(.system(size:10)).foregroundStyle(Palette.muted)
                Text(verbatim:tr("Geschwindigkeit und Nachleuchten steuert der Govee-Modus selbst. Separate Regler dafür oder für ein Musik-Mindestlicht bietet diese API nicht.")).font(.system(size:10)).foregroundStyle(Palette.muted)
            }.padding(22)
    ...
    private func control(_ title: String,value: Binding<Double>,range: ClosedRange<Double>,label: String) -> some View {
        VStack(spacing:9) {
            HStack { Text(verbatim:tr(title)).font(.system(size:11,weight:.medium)); Spacer(); Text(verbatim:tr(label)).font(.system(size:10,design:.monospaced)).foregroundStyle(Palette.mint) }
